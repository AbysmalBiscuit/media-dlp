...
test('the video id appears last, using the configured separator', () => {
  const f = chips({ 'video-id': true, separator: 'underscore' });
  expect(previewFilename(f, info, 'mp4')).toBe('A Video_dQw4w9WgXcQ.mp4');
});

test('the video id chip is absent when its flag is off', () => {
  expect(previewFilename(chips(), info, 'mp4')).toBe('A Video.mp4');
});

test('an eight-digit upload date is rendered as YYYY-MM-DD', () => {
  const f = chips({ 'upload-date': true });
  expect(previewFilename(f, info, 'mp4')).toBe('2026-02-14 - A Video.mp4');
});

test('an upload date that is not eight digits passes through untouched', () => {
  const odd: ProbeInfo = { ...info, uploadDate: 'NA' };
  const f = chips({ 'upload-date': true });
  expect(previewFilename(f, odd, 'mp4')).toBe('NA - A Video.mp4');
});
